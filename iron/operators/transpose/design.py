...
def shuffle_transpose(
    ...
    in_prods = [of.prod() for of in of_in1s_L3L2]
    out_conses = [of.cons() for of in of_outs]

    def sequence(A, C, in_hs, out_hs):
        ...
        for batch in range(num_batches):
            ...
            for i in range(num_columns):
                for j in range(num_channels):
                    in_hs[i * num_channels + j].fill(
                        A, taps_in_L3L2[i * num_channels + j][batch], group=tg
                    )
            ...
            for i in range(num_columns):
                for j in range(num_channels):
                    ...
                    out_hs[i * num_channels + j].drain(
                        C, taps_out_L1L3[i * num_channels + j][batch], wait=True, group=tg
                    )
            tg.finish()

    rt = Runtime(sequence, [tensor_ty, tensor_ty, in_prods, out_conses])
